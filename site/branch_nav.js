...
export function bindBranchNav(root = document) {
  ...
  shells.forEach((shell) => {
    const toggle = shell.querySelector("[data-branch-nav-toggle]");
    const menu = shell.querySelector("[data-branch-nav-menu]");
    const label = shell.querySelector("[data-branch-nav-toggle-label]");
    const links = [...shell.querySelectorAll("[data-branch-nav-link]")];
    const defaultLabel = label?.textContent?.trim() || "Branches";
    let open = false;
    const dropdown = {
      close: () => setOpen(false),
    };
